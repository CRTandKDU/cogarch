...
bool cb_sort_onname(sign_rec_ptr si, sign_rec_ptr sj) {
	return (strcmp(si->str, sj->str) < 0);
}
...
// Derive a class from Fl_Table
	...
	//
void EncyTable::DrawHeader(const char* s, int X, int Y, int W, int H) {
...
//
void EncyTable::draw_cell(TableContext context, int ROW, int COL, 
...
void EncyTable::clear() { data.clear(); }

int EncyTable::fill(sign_rec_ptr first) {
	sign_rec_ptr sign;
	int row = 0;
	clear();
	top = first;
	...
	std::sort(data.begin(), data.end(), cb_sort_onname);
...
EncyTable::EncyTable(sign_rec_ptr first, EncyTable::item _item_t,
	...
	col_width_all(tow >> 1);          // default width of columns
...
EncyWin::EncyWin(sign_rec_ptr top, EncyTable::item _item_t) : Fl_Double_Window(400, 400, "Encyclopedia") {
	table = new EncyTable(top, _item_t, 10, 10, 380, 380);
	end();
	resizable(table);
}
EncyWin::~EncyWin() {
