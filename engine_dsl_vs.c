...
extern std::string repl_getLastdir();
#endif
...
int nxpcsv_io( char *fn, csv_cb1_t f_cb1, csv_cb2_t f_cb2 ){
  ...
  S_csv_col	= 0;
#ifdef FLTK
  std::string fullfn = repl_getLastdir() + std::string(fn);
  std::replace(fullfn.begin(), fullfn.end(), '/', '\\');
  fp = fopen(fullfn.c_str(), "rb");
#else
  fp = fopen(fn, "rb");
#endif
  if (!fp) {
...
static int cb_nxpshow(vm_extension_t * const v) {
  ...
  cell_t         val;
#ifdef FLTK
  std::string cmdstr = std::string( "rundll32.exe \"C:\\Program Files\\Windows Photo Viewer\\PhotoViewer.dll\", ImageView_Fullscreen " );
  char buf[_MARSHALL_BUFLEN];
  std::string fullfn;
  res = marshall_forth_compactstring(buf, v);
  fullfn = repl_getLastdir() + std::string(buf);
  std::replace(fullfn.begin(), fullfn.end(), '/', '\\');
  cmdstr = cmdstr + fullfn;
  fprintf(stderr, "%s\n", cmdstr.c_str());
  system(cmdstr.c_str());
#else
...
int  engine_dsl_eval_async( const char * exp, int *err, int *suspend ){
  ...
  repl_log("<FORTH> Evaluating: %s (Suspend %d)", expr, *suspend);

  S_v->suspend = suspend;

  int r = embed_eval( S_v->h, expr );
  repl_log("<FORTH> Eval =  %d (Suspend %d)", r, *suspend);
  
  r = embed_pop( S_v->h, &val );

  *err = r;
  repl_log("<FORTH> Err = %d (Suspend %d)", r, *suspend);
