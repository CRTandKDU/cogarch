...
int nxp_init( vm_extension_t * const v ){
  ...
  char prgm[80];
  int  r = 0;
  sprintf( prgm, templ_sign_decl, "task", "task" );
  r = embed_eval( v->h, prgm );
  sprintf( prgm, templ_sign_decl, "crt_and_kdu", "crt_and_kdu" );
  r = embed_eval( v->h, prgm );
  return r;
}

int main(void) {
  char prgm[] = "task nxp@ 10 + . crt_and_kdu nxp@ 10 + . cr .s\n";
